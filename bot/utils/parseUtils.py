...
async def get_message_data(m: Message, isInput: bool = False):
    data_type = None
    content = None
    ...
    if not m.reply_to_message and m.text and len(m.text.split()) >= 2:
        ...
        if len(args) >= 1 and m.reply_to_message.text:  # not caption, text
            ...
            data_type = m.reply_to_message.media.value
            try:
                content = (getattr(m.reply_to_message, data_type)).file_id
    ...
    if isInput:
        ...
        if len(args) >= 1 and m.text:  # not caption, text
            data_type = "text"
        elif m.media:
            data_type = m.media.value
            try:
                content = (getattr(m, data_type)).file_id
            except AttributeError:
                content = None
